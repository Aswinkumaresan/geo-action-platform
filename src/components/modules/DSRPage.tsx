...
// ── BL Goondas ────────────────────────────────────────────────────────────────
const goondasData = [
  { name: "Raju Bhai",       alias: "RB",    district: "Chennai",    cases: 5, lastOffence: "Liquor smuggling",      status: "Active" },
  { name: "Srinivas K",      alias: "Srinu", district: "Coimbatore", cases: 3, lastOffence: "Ganja trafficking",     status: "Detained" },
  { name: "Mohammed Ismail", alias: "MI",    district: "Madurai",    cases: 8, lastOffence: "Repeat offender - NDPS",status: "Active" },
  { name: "Venkatesh P",     alias: "Venky", district: "Chennai",    cases: 2, lastOffence: "Solvent misuse",        status: "Released" },
];
...
// ── Bandobust ─────────────────────────────────────────────────────────────────
const bandobustData = [
  { op: "Op Thunderbolt", date: "2026-03-04", unit: "Unit A", personnel: 12, status: "In Progress", result: "2 seizures" },
  { op: "Op Clean Sweep", date: "2026-03-03", unit: "Unit B", personnel: 8,  status: "Completed",   result: "5 arrests, 100L liquor" },
  { op: "Op Night Watch", date: "2026-03-02", unit: "Unit C", personnel: 15, status: "Completed",   result: "3 checkpost intercepts" },
];
...
// ── Vehicle Disposal ──────────────────────────────────────────────────────────
const vehicleData = [
  { no: "TN 01 AB 1234", type: "Truck",       seized: "2026-02-15", caseRef: "CS-2026-045", location: "Unit A Yard",     status: "In Custody",          disposition: "-" },
  { no: "TN 09 CD 5678", type: "Two-wheeler", seized: "2026-01-20", caseRef: "CS-2026-021", location: "Court Premises",  status: "Court Order Pending",  disposition: "-" },
  { no: "TN 07 EF 9012", type: "Auto",        seized: "2025-12-10", caseRef: "CS-2025-198", location: "Auction Yard",    status: "Disposed",             disposition: "Auctioned" },
  { no: "TN 05 GH 3456", type: "Car",         seized: "2026-02-28", caseRef: "CS-2026-067", location: "Unit B Yard",     status: "In Custody",           disposition: "-" },
];
...
// ── Solvent Inspections ───────────────────────────────────────────────────────
const solventData = [
  { licensee: "ABC Chemicals Ltd", licenseNo: "SOL-2025-001", type: "Solvent",  date: "2026-03-03", inspector: "SI Ramesh", result: "Compliant", remarks: "All records in order" },
  { licensee: "XYZ Industries",    licenseNo: "MET-2025-045", type: "Methanol", date: "2026-03-02", inspector: "CI Sharma", result: "Violation", remarks: "Excess stock found, show caus..." },
  { licensee: "PQR Solutions",     licenseNo: "SOL-2025-078", type: "Solvent",  date: "2026-03-01", inspector: "SI Reddy",  result: "Compliant", remarks: "Minor documentation gaps no..." },
];
...
const helplineData = [
  { time: "09:15", source: "Helpline",     caller: "Anonymous",  type: "Tip-off",      summary: "Illegal liquor transport on NH-65",       assigned: "Unit A", status: "Actioned" },
  { time: "10:30", source: "Control Room", caller: "SI Ramesh",  type: "Intelligence", summary: "Ganja cultivation spotted in fores...",    assigned: "Unit B", status: "Pending" },
  { time: "11:45", source: "Helpline",     caller: "Public",     type: "Complaint",    summary: "Illegal bar operating without lice...",    assigned: "Unit C", status: "Under Review" },
  { time: "14:00", source: "Control Room", caller: "DSP Office", type: "Directive",    summary: "Intensify checking on state border",       assigned: "All Units", status: "Actioned" },
];
          ...
          <div className="rounded-b-lg border border-t-0 overflow-hidden" style={{ borderColor: "hsl(var(--border))" }}>
            <div className="p-4">
              ...
              {activeTab === "BL Goondas Entry" && (
                <>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base font-bold text-foreground">BL Goondas Registry</h3>
                    <button className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold" style={{ background: "hsl(var(--primary))", color: "hsl(var(--primary-foreground))" }}><Plus className="h-4 w-4" /> New Entry</button>
                  </div>
                  <div className="relative mb-4 max-w-xs">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                    <input placeholder="Search by name or alias..." className="w-full rounded-lg border pl-9 pr-4 py-2 text-sm bg-background focus:outline-none" style={{ borderColor: "hsl(var(--border))" }} />
                  </div>
                  <table className="w-full text-sm">
                    <thead><tr className="border-b" style={{ borderColor: "hsl(var(--border))" }}>
                      {["Name","Alias","District","Cases","Last Offence","Status"].map(c => <th key={c} className="text-left py-3 px-3 text-muted-foreground font-medium text-xs">{c}</th>)}
                    </tr></thead>
                    <tbody>
                      {goondasData.map(r => (
                        <tr key={r.name} className="border-b hover:bg-muted/30 transition-colors" style={{ borderColor: "hsl(var(--border))" }}>
                          <td className="py-3 px-3 font-semibold">{r.name}</td>
                          <td className="py-3 px-3 text-muted-foreground">{r.alias}</td>
                          <td className="py-3 px-3">{r.district}</td>
                          <td className="py-3 px-3">{r.cases}</td>
                          <td className="py-3 px-3">{r.lastOffence}</td>
                          <td className="py-3 px-3"><span className={`px-3 py-1 rounded-full text-xs font-semibold ${goondasStatusColor[r.status]}`}>{r.status}</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {activeTab === "Bandobust Details" && (
                <>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base font-bold text-foreground">Bandobust Details</h3>
                    <button className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold" style={{ background: "hsl(var(--primary))", color: "hsl(var(--primary-foreground))" }}><Plus className="h-4 w-4" /> New Operation</button>
                  </div>
                  <div className="rounded-lg border overflow-hidden" style={{ borderColor: "hsl(var(--border))" }}>
                    <div className="px-4 py-3 border-b text-sm font-semibold text-foreground" style={{ borderColor: "hsl(var(--border))" }}>Operations Log</div>
                    <table className="w-full text-sm">
                      <thead><tr className="border-b" style={{ borderColor: "hsl(var(--border))" }}>
                        {["Operation","Date","Unit","Personnel","Status","Result"].map(c => <th key={c} className="text-left py-3 px-4 text-muted-foreground font-medium text-xs">{c}</th>)}
                      </tr></thead>
                      <tbody>
                        {bandobustData.map(r => (
                          <tr key={r.op} className="border-b hover:bg-muted/30 transition-colors" style={{ borderColor: "hsl(var(--border))" }}>
                            <td className="py-3 px-4 font-semibold">{r.op}</td>
                            <td className="py-3 px-4 text-muted-foreground">{r.date}</td>
                            <td className="py-3 px-4" style={{ color: "hsl(var(--primary))" }}>{r.unit}</td>
                            <td className="py-3 px-4">{r.personnel}</td>
                            <td className="py-3 px-4"><span className={`px-3 py-1 rounded-full text-xs font-semibold ${bbStatusColor[r.status]}`}>{r.status}</span></td>
                            <td className="py-3 px-4 text-muted-foreground">{r.result}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {activeTab === "Vehicle Disposal" && (
                <>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base font-bold text-foreground">Vehicle Disposal Tracking</h3>
                    <button className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold" style={{ background: "hsl(var(--primary))", color: "hsl(var(--primary-foreground))" }}><Plus className="h-4 w-4" /> Add Vehicle</button>
                  </div>
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    {[{ label:"Total Seized", value: vehicleData.length, color:"hsl(var(--primary))" }, { label:"In Custody", value: vehicleData.filter(v=>v.status==="In Custody").length, color:"hsl(var(--risk-medium))" }, { label:"Disposed", value: vehicleData.filter(v=>v.status==="Disposed").length, color:"hsl(var(--risk-low))" }].map(s => (
                      <div key={s.label} className="rounded-lg border p-4 text-center" style={{ borderColor: "hsl(var(--border))" }}>
                  ...
                  <table className="w-full text-sm">
                    <thead><tr className="border-b" style={{ borderColor: "hsl(var(--border))" }}>
                      {["Vehicle No.","Type","Seized","Case Ref","Location","Status","Disposition"].map(c => <th key={c} className="text-left py-3 px-3 text-muted-foreground font-medium text-xs">{c}</th>)}
                    </tr></thead>
                    <tbody>
                      {vehicleData.map(r => (
                        <tr key={r.no} className="border-b hover:bg-muted/30 transition-colors" style={{ borderColor: "hsl(var(--border))" }}>
                          <td className="py-3 px-3 font-mono text-xs">{r.no}</td>
                          <td className="py-3 px-3">{r.type}</td>
                          <td className="py-3 px-3 text-muted-foreground">{r.seized}</td>
                          <td className="py-3 px-3 font-mono text-xs">{r.caseRef}</td>
                          <td className="py-3 px-3">{r.location}</td>
                          <td className="py-3 px-3"><span className={`px-3 py-1 rounded-full text-xs font-semibold ${vehicleStatusColor[r.status]}`}>{r.status}</span></td>
                          <td className="py-3 px-3 text-muted-foreground">{r.disposition}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {activeTab === "Solvent Inspections" && (
                <>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base font-bold text-foreground">Solvent/Methanol License Inspection</h3>
                    <button className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold" style={{ background: "hsl(var(--primary))", color: "hsl(var(--primary-foreground))" }}><Plus className="h-4 w-4" /> New Inspection</button>
                  </div>
                  <div className="rounded-lg border overflow-hidden" style={{ borderColor: "hsl(var(--border))" }}>
                    <table className="w-full text-sm">
                      <thead><tr className="border-b" style={{ borderColor: "hsl(var(--border))" }}>
                        {["Licensee","License No.","Type","Date","Inspector","Result","Remarks"].map(c => <th key={c} className="text-left py-3 px-4 text-muted-foreground font-medium text-xs">{c}</th>)}
                      </tr></thead>
                      <tbody>
                        {solventData.map(r => (
                          <tr key={r.licenseNo} className="border-b hover:bg-muted/30 transition-colors" style={{ borderColor: "hsl(var(--border))" }}>
                            <td className="py-3 px-4 font-semibold">{r.licensee}</td>
                            <td className="py-3 px-4 font-mono text-xs text-muted-foreground">{r.licenseNo}</td>
                            <td className="py-3 px-4">{r.type}</td>
                            <td className="py-3 px-4 text-muted-foreground">{r.date}</td>
                            <td className="py-3 px-4">{r.inspector}</td>
                            <td className="py-3 px-4"><span className={`px-3 py-1 rounded-full text-xs font-semibold ${solventResultColor[r.result]}`}>{r.result}</span></td>
              ...
              {activeTab === "Helpline Calls" && (
                <>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base font-bold text-foreground">Control Room & Helpline</h3>
                    <button className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold" style={{ background: "hsl(var(--primary))", color: "hsl(var(--primary-foreground))" }}><Plus className="h-4 w-4" /> Log Call</button>
                  </div>
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    {[{ label:"Total Calls", value:27, color:"hsl(var(--primary))" }, { label:"Actioned", value:18, color:"hsl(var(--risk-low))" }, { label:"Pending", value:9, color:"hsl(var(--risk-medium))" }].map(s => (
                      <div key={s.label} className="rounded-lg border p-4 text-center" style={{ borderColor: "hsl(var(--border))" }}>
                        <p className="text-2xl font-bold" style={{ color: s.color }}>{s.value}</p>
                        <p className="text-xs text-muted-foreground mt-1">{s.label}</p>
                      </div>
                  ...
                  <div className="rounded-lg border overflow-hidden" style={{ borderColor: "hsl(var(--border))" }}>
                    <table className="w-full text-sm">
                      <thead><tr className="border-b" style={{ borderColor: "hsl(var(--border))" }}>
                        {["Time","Source","Caller","Type","Summary","Assigned","Status"].map(c => <th key={c} className="text-left py-3 px-4 text-muted-foreground font-medium text-xs">{c}</th>)}
                      </tr></thead>
                      <tbody>
                        {helplineData.map((r, i) => (
                          <tr key={i} className="border-b hover:bg-muted/30 transition-colors" style={{ borderColor: "hsl(var(--border))" }}>
                            <td className="py-3 px-4 font-mono text-xs">{r.time}</td>
                            <td className="py-3 px-4" style={{ color: r.source === "Helpline" ? "hsl(var(--risk-high))" : undefined }}>{r.source}</td>
                            <td className="py-3 px-4">{r.caller}</td>
                            <td className="py-3 px-4"><span className="px-2 py-0.5 rounded border text-xs" style={{ borderColor: "hsl(var(--border))" }}>{r.type}</span></td>
                            <td className="py-3 px-4 text-muted-foreground">{r.summary}</td>
                            <td className="py-3 px-4" style={{ color: "hsl(var(--primary))" }}>{r.assigned}</td>
                            <td className="py-3 px-4"><span className={`px-3 py-1 rounded-full text-xs font-semibold ${helplineStatusColor[r.status]}`}>{r.status}</span></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
