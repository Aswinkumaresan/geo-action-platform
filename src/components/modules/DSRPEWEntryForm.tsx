...
export function DSRPEWEntryForm({ onClose }: { onClose: () => void }) {
  ...
  const [seizureRows, setSeizureRows] = useState<Record<string, { cases: string; quantity: string; accused: string }>>(
    Object.fromEntries(seizureCategories.map(c => [c.label, { cases: "", quantity: "", accused: "" }]))
  );
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-1 overflow-x-auto pb-1">
        {steps.map((step, i) => (
          <button
            ...
            style={i === activeStep ? { background: "hsl(var(--primary))" } : undefined}
          >
            {step}
          </button>
        ))}
      ...
      {activeStep === 0 && (
        <div className="space-y-5">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b" style={{ borderColor: "hsl(var(--border))" }}>
                  <th className="text-left py-2 px-2 text-xs font-semibold text-muted-foreground" style={{ width: "45%" }}>Category</th>
              ...
              <tbody>
                {seizureCategories.map((cat, i) => (
                  <tr key={cat.label} className={i % 2 === 0 ? "bg-muted/20" : ""}>
                    <td className="py-1.5 px-2 text-sm font-medium">{cat.label} <span className="text-muted-foreground font-normal">({cat.unit})</span></td>
                    <td className="py-1 px-1"><input value={seizureRows[cat.label].cases} onChange={e => updateSeizureRow(cat.label, "cases", e.target.value)} className="w-full rounded border px-2 py-1.5 text-xs text-center bg-background focus:outline-none focus:ring-1 focus:ring-primary" style={{ borderColor: "hsl(var(--border))" }} /></td>
                    <td className="py-1 px-1"><input value={seizureRows[cat.label].quantity} onChange={e => updateSeizureRow(cat.label, "quantity", e.target.value)} className="w-full rounded border px-2 py-1.5 text-xs text-center bg-background focus:outline-none focus:ring-1 focus:ring-primary" style={{ borderColor: "hsl(var(--border))" }} /></td>
                    <td className="py-1 px-1"><input value={seizureRows[cat.label].accused} onChange={e => updateSeizureRow(cat.label, "accused", e.target.value)} className="w-full rounded border px-2 py-1.5 text-xs text-center bg-background focus:outline-none focus:ring-1 focus:ring-primary" style={{ borderColor: "hsl(var(--border))" }} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {/* Vehicle Seizure */}
          <div className={sectionCls} style={{ borderColor: "hsl(var(--border))" }}>
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wide mb-3">Vehicle Seizure Details</p>
      ...
      {activeStep === 2 && (
        <div className="space-y-5">
          ...
          <div className="flex justify-between">
            ...
            <button onClick={() => setActiveStep(3)} className="px-6 py-2.5 rounded-lg text-sm font-semibold" style={{ background: "hsl(var(--primary))", color: "hsl(var(--primary-foreground))" }}>Next: Court Hearing →</button>
          </div>
        </div>
      )}
